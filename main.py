from tkinter import*
import sqlite3
filename = 'meat_consumption.csv'
a=Tk()
a.title("The Dietician")
a.geometry("700x500")
con=sqlite3.connect("diet1.db")
con.execute("create table if not exists diet(height text,gender text,weight text,age text,activity text)")
...
def BMR():
    ...
    with con:
        cursor=con.cursor()
        cursor.execute("insert into diet values(?,?,?,?,?)",(h,gender,w,age,act))
        b2 = Button(top, text = 'next', width=25)
        b2.grid()
